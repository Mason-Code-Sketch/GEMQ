...
def _load_split(dataset_root, dataset_name, split):
    """Load one split from the local DatasetDict layout when configured."""
    ...
    try:
        dataset_dir = Path(dataset_root) / directory_names[dataset_name]
...
def get_loaders(
    ...
    if "wikitext2" in name:
        return get_wikitext2(
            nsamples, seed, seqlen, model, use_fast, dataset_root
        )
    if "c4" in name:
