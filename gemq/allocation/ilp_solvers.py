...
class GEMQSolver:
    ...
    def __init__(
        ...
        for expert_id, bitwidth in self.fixed_expert_bits.items():
            if not 0 <= expert_id < self.num_experts:
                raise ValueError(f"Fixed expert id {expert_id} is out of range for {self.num_experts} experts")
            if bitwidth not in self.x_space:
                raise ValueError(f"Fixed bitwidth {bitwidth} is not in x_space={self.x_space}")
    ...
    def build_constraints(self, total_bits):
        ...
        if self.fixed_expert_bits:
            ...
            A_fixed = sp.csr_matrix((np.ones(len(rows)), (rows, cols)), shape=(len(rows), n))
